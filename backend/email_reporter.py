"""
Daily email reporter for SerenityAlphaTrader.
...
IMAP_PORT = 993
REPORT_SUBJECT_PREFIX = "SerenityAlphaTrader Daily Report"
...
<div style="max-width:720px;margin:24px auto;background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
  ...
  <div style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 100%);padding:28px 32px;color:#fff;">
    <div style="font-size:22px;font-weight:700;letter-spacing:1px;">📈 SerenityAlphaTrader Daily Report</div>
  ...
  <div style="background:#f8f9fa;padding:16px 32px;text-align:center;color:#aaa;font-size:11px;border-top:1px solid #eee;">
    SerenityAlphaTrader Pro · 本报告由 AI 自动生成，不构成投资建议 · 请独立判断风险
  </div>
...
def send_email(sender: str, app_password: str, recipient: str, subject: str, html: str) -> bool:
    ...
    try:
        ...
        msg["To"] = recipient
        msg["X-Mailer"] = "SerenityAlphaTrader-Pro"
...
def _fetch_new_replies(mail: imaplib.IMAP4_SSL) -> list[dict]:
    """
    After IDLE notifies of new mail, fetch any unread SerenityAlphaTrader reply messages.
    Returns list of {subject, body, date} dicts.
    """
    replies = []
    try:
        # Search for unseen replies with SerenityAlphaTrader in subject
        status, data = mail.search(None, '(UNSEEN SUBJECT "SerenityAlphaTrader")')
...
async def process_reply_with_ai(reply_body: str, db, settings: dict) -> dict:
    ...
    try:
        ...
        prompt = f"""You are the control system for an automated stock trading platform called SerenityAlphaTrader.
