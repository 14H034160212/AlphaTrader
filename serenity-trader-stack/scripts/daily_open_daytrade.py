...
def pick_todays_stocks(api, exclude=None, extra_note=""):
    ...
    prompt = (
        ...
        "美债完全可以接受,不需要为了交易而交易。\n\n"
        "**这条规则也适用于单只候选,不只是选几只这个整体决定**(2026-08-11,"
        "用户复盘时发现EMBJ的问题——它的理由里自己写了'全年指引仅重申且低于"
        "预期'这种实质性反向证据,权重却排在CXW、TEM这些理由干净、没有反向"
        "证据的候选前面,说明排序被headline数字大小带偏了,而不是被论文质量"
        "带动):写理由的过程中如果你自己发现了实质性的反向证据(指引不及预期、"
        "催化剂本身无法证实、涨幅已经透支等),不是'降权重意思一下'就够了——"
        "直接排除这只,对应的资金留给SPY,不要为了凑数硬塞一个连你自己都在"
        "怀疑的标的。**注意区分'反向证据'和'结果不确定'**:FDA专家委员会已"
        "定档投票但你不知道会不会通过、诉讼判决日期已定但不知道输赢——这种"
        "binary事件型催化剂,只要事件本身真实、日期明确,不算反向证据,可以"
        "正常入选,只需要用较小仓位控制这种不确定性,不必因为'不知道结果'"
        "就直接排除。\n\n"
        "**合同流型催化剂必须给最高档权重**(2026-08-11/12两天复盘都验证):"
        "已签约对手方+跨季度期限+付款方是政府或已签约大客户+卖方给出具体目标价"
        "区间(比如CXW的联邦拘留合同,两家投行目标价$42-45)——这类是实测中"
        "唯一两天都跑赢大盘的类型,必须排在'财报超预期'或'分析师上调目标价'"
        "这类偏软的催化剂前面拿最高档权重,不能因为财报beat的数字更大就把"
        "合同流型放到中低档。\n\n"
        "**建仓前必须检查催化剂的时效性和头号反向风险**(2026-08-11复盘:GRAL"
        "的FDA专家委员会定档消息是4个交易日前就公布的,建仓时'去风险'早被"
        "消化完;真正在跌的原因是当季净亏$110.2M、年化烧钱量大、有增发摊薄"
        "风险,但买入理由里完全没提这件事):消息公布超过2个交易日的,视为"
        "已经price-in,不再作为新建仓理由(除非有新的独立触发);同时必须"
